                             ...
                             const vector_t& rbdStateMeasured, size_t mode, scalar_t period)
{
  ...
  // Cost
  Task weighedTask = formulateWeightedTasks(inputDesired);
...
Task WeightedWbc::formulateWeightedTasks(const vector_t& inputDesired)
{
  if (stance_mode_)
    return formulateStanceBaseAccelTask() * weightBaseAccel_;
  else
    return formulateSwingLegTask() * weightSwingLeg_ +    //只有摆动腿轨迹
           formulateBaseAccelTask() * weightBaseAccel_ +
           formulateContactForceTask(inputDesired) * weightContactForce_;
}

Task WeightedWbc::formulateStanceBaseAccelTask()
{
