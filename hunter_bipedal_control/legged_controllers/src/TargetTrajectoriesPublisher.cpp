...
scalar_t estimateTimeToTarget(const vector_t& desiredBaseDisplacement)  //预估到达目的地时间，旋转和直线运动时间的取最大值
{
                                                  ...
                                                  const scalar_t& targetReachingTime)
{
  ...
  const vector_array_t inputTrajectory(2, vector_t::Zero(observation.input.size()));  //期望轨迹中输入全0
...
TargetTrajectories goalToTargetTrajectories(const vector_t& goal, const SystemObservation& observation)
{
  ...
  const vector_t targetPose = [&]() {
    ...
    scalar_t dz = BASE_HEIGHT - currentPose(2);
    dz = dz > 0 ? fmin(dz, changeLimit_[2]) : fmax(dz, -changeLimit_[2]);
    target(2) = currentPose(2) + dz;    //高度变化不超过changeLimit_[2]
    target(3) = goal(3);
    target(4) = 0;
    target(5) = 0;
    return target;
  }();
  const scalar_t targetReachingTime = observation.time + estimateTimeToTarget(targetPose - currentPose);//预估到达时间
  return targetPoseToTargetTrajectories(targetPose, observation, targetReachingTime); //生成期望轨迹，只包含起始和到达两个时刻
}
...
TargetTrajectories cmdVelToTargetTrajectories(const vector_t& cmdVel, const SystemObservation& observation)
{
  ...
  vector_t cmdVelRot = getRotationMatrixFromZyxEulerAngles(zyx) * cmdVel.head(3);   //cmdVel是相对机器人正前方的速度，转为世界坐标系下的速度
  const scalar_t timeToTarget = TIME_TO_TARGET;   //MPC预测时长
  ...
  trajectories.stateTrajectory[1].head(3) = cmdVelRot;  //生成期望轨迹，只包含起始和到达两个时刻，期望轨迹状态变量前3维是期望速度
  return trajectories;
}
...
int main(int argc, char** argv)
{
  ...
  loadData::loadCppDataType(referenceFile, "baseHeight", BASE_HEIGHT);
